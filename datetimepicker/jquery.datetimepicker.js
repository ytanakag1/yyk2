...
var disableDate = new Date();         // 現在日時を生成
var yyyy = disableDate.getFullYear(); // 西暦を取得
var mm = disableDate.getMonth() + 1;  // 月を取得（返り値は実際の月-1なので、+1する）
var dd = disableDate.getDate(); // 日を取得
var w = disableDate.getDay();   // 曜日を取得（数値）
...
var disableDate = dd + "." + mm + "." + yyyy ; // フォーマットを整えて表示
	...
	var cdate= start.dateFormat('d.m.Y');

							if( ( maxDate!==false && start > maxDate )||(  minDate!==false && start < minDate ) 
								|| start.dateFormat('d.m.Y')==disableDate){
						...
						var time = '',
							...
							line_time = function line_time( h,m ) {
								...
								if( (options.maxTime!==false 
									&& _xdsoft_datetime.strtotime(options.maxTime).getTime()<now.getTime())
